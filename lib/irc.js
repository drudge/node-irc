        ...
        switch ( message.command ) {
            ...
            case "err_nicknameinuse":
                if ( typeof(self.opt.nickMod) == 'undefined' )
                    self.opt.nickMod = 0;
                self.opt.nickMod++;
                self.setNick(self.opt.nick + self.opt.nickMod);
                break;
            case "PING":
            ...
            case "MODE":
                ...
                if ( message.args.length >= 3 ) {
                    ...
                    self.emit('mode', message.nick, message.args[1], message.args[0], message.args.slice(2), message);
                    }
                break;
            case "NICK":
                if ( self.opt.debug )
            ...
            case "err_nomotd":
                self.motd += message.args[1] + "\n";
                self.emit('motd', self.motd, message);
                break;
            case "rpl_namreply":
            ...
            case "rpl_endofnames":
                ...
                self.emit('names', message.args[1], channel.users, message);
            ...
            case "TOPIC":
                // channel, topic, nick
                self.emit('topic', message.args[0], message.args[1], nick, message);
            ...
            case "PRIVMSG":
                ...
                if ( to.match(/^[&#]/) ) self.emit('message' + to, from, text, message);
...
Client.prototype.act = function(target, text) { // {{{
    this.send('PRIVMSG', target, '\001ACTION' + text + '\001');
} // }}}
Client.prototype.setTopic = function(target, text) { // {{{
    this.send('TOPIC', target, text);
} // }}}
Client.prototype.setNick = function(newNick) { // {{{
